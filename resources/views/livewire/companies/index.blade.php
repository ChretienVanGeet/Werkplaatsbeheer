...
{{--                <flux:table.column sortable :sorted="$sortBy === 'locations'" :direction="$sortDirection" wire:click="sort('locations')">{{ __('Locations') }}</flux:table.column>--}}
                ...
                @foreach ($this->rows() as $company)
                    <flux:table.row :key="$company->id">
                        ...
                        <x-tables.action-column
                            :model="$company"
                            :deleteClick="'confirmDelete('.$company->id.')'"
                            :editRoute="route('companies.edit', $company)"
                            :showRoute="route('companies.show', $company)"
                            :noteClick="'openNotes('.$company->id.')'"
                        />
                    </flux:table.row>
                @endforeach
            </flux:table.rows>
    ...
    <flux:modal name="company-notes" variant="floating" flyout>
